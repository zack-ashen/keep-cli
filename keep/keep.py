...
fig = Figlet(font='larry3d', justify='center', width=consts.WIDTH)
keep = gkeepapi.Keep()
...
def note_edit_view(note_to_edit, google_notes, note_list, index_of_note):
    ...
    NoteGrid.print_grid(note_to_edit, continue_printing_row)
    continue_printing_row = True
    ...
    if list_selection.get('options') == consts.DELETE_NOTE:
        note_to_delete = google_notes[index_of_note]
        note_to_delete.delete()
        keep.sync()
        note_view()
    elif list_selection.get('options') == consts.EDIT_TITLE:
        new_title_prompt = [{
                'type': 'input',
                'name': 'noteTitle',
                'message': consts.MAKE_TITLE,
        ...
        note_edit_view(note_to_edit, google_notes, note_list, index_of_note)
    elif list_selection.get('options') == consts.EDIT_BODY:
        ...
        keep.sync()
        note_view()
    elif list_selection.get('options') == consts.EDIT_ITEMS:
        item_list = []
        ...
        item_list.append('...' + consts.ADD_ITEMS)
        item_list.append('...' + consts.GO_BACK)

        item_to_edit = [
            {
                'type': 'list',
                'name': 'item',
                'message': consts.SELECT_ITEM,
                'choices': item_list
            }]

        item_to_edit_answer = prompt(item_to_edit)

        if item_to_edit_answer.get('item') == '...' + consts.ADD_ITEMS:
            list_finished = False
            list_items = []
            while not list_finished:
                add_list_item = [
        ...
        elif item_to_edit_answer.get('item') == '...' + consts.GO_BACK:
            note_edit_view(note_to_edit, google_notes, note_list, index_of_note)
        else:
        ...
        note_edit_view(note_to_edit, google_notes, note_list, index_of_note)
    elif list_selection.get('options') == consts.CHECK_ITEMS:
        ...
        note_to_edit[0] = note_list[index_of_note]
        note_edit_view(note_to_edit, google_notes, note_list, index_of_note)
    elif list_selection.get('options') == consts.GO_BACK:
        edit_note_selector_view(note_list, google_notes)
    ...
    username_prompt = [
        {
            'type': 'input',
            'name': 'username',
            'message': consts.USERNAME_PROMPT,
        }]
        ...
        password_credentials = [
            {
                'type': 'password',
                'name': 'password',
                'message': consts.PASSWORD_PROMPT,
            },
            {
                'type': 'confirm',
                'name': 'confirm-save',
                'message': consts.SAVE_PASSWORD
            }]
...
def animate_welcome_text():
    ...
    if consts.WIDTH < 100:
        ...
        paragraph_text = str(fill(paragraph_text, consts.WIDTH / 2))
        paragraph_text_list = paragraph_text.split('\n')
        for index in range(len(paragraph_text_list)):
            print(paragraph_text_list[index].center(consts.WIDTH))
        print('\n')

        line = ''
        for index in range(consts.WIDTH):
            line += '─'
        print(line)
